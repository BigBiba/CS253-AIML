...
{
    public enum AlphabetLetter : byte
    {
        Alpha = 0,
        Beta,
        Chi,
    ...
    public class Sample
    {
        ...
        /// </summary>
        public AlphabetLetter actualClass;

        /// <summary>
        /// Распознанный класс - определяется после обработки
        /// </summary>
        public AlphabetLetter recognizedClass;
        ...
        public Sample(double[] inputValues, int classesCount, AlphabetLetter sampleClass = AlphabetLetter.Undef)
        {
            //  Клонируем массивчик
            input = (double[])inputValues.Clone();
            Output = new double[classesCount];
            if (sampleClass != AlphabetLetter.Undef) Output[(int)sampleClass] = 1;


            recognizedClass = AlphabetLetter.Undef;
        ...
        /// </summary>
        public AlphabetLetter ProcessPrediction(double[] neuralOutput)
        {
            Output = neuralOutput;
            if (error == null)
            ...
            for (int i = 0; i < Output.Length; ++i)
            {
                ...
                if (Output[i] > Output[(int)recognizedClass]) recognizedClass = (AlphabetLetter)i;
            }
